...
def main():
    ...
    # 1. Load evaluation set (mapping keys from val_split)
    eval_data = []
    ...
    mistral_base_path = "mistralai/Mistral-7B-Instruct-v0.3"
    mistral_path = "./trained_models/mistral-7b-cmg-qlora_merged"
    qwen_path = "./trained_models/qwen-7b-cmg-qlora_merged"
    
    results = {}
    
    print("Evaluating Mistral Base (mistralai/Mistral-7B-Instruct-v0.3)...")
    try:
        results["mistral_base"] = evaluate_model(
            mistral_base_path, eval_data, db_data, faiss_index, embed_model, embed_tokenizer, reranker, scorer, device
        )
    except Exception as e:
        print(f"Error evaluating Mistral Base: {e}")
        
    if os.path.exists(mistral_path):
        results["mistral_tuned"] = evaluate_model(
            mistral_path, eval_data, db_data, faiss_index, embed_model, embed_tokenizer, reranker, scorer, device
        )
    else:
        print(f"Warning: Mistral tuned model not found at {mistral_path}. Skipping.")
        
    if os.path.exists(qwen_path):
        results["qwen_tuned"] = evaluate_model(
            qwen_path, eval_data, db_data, faiss_index, embed_model, embed_tokenizer, reranker, scorer, device
        )
    else:
        print(f"Warning: Qwen tuned model not found at {qwen_path}. Skipping.")
        
    # Print and generate report
    report_lines = [
        "# DWP CMG RAG Validation Split Evaluation Report",
        f"\nThis report compares the performance of the baseline Mistral-7B-Instruct-v0.3, fine-tuned Mistral-7B-Instruct-v0.3, and fine-tuned Qwen-2.5-7B-Instruct models on the full validation split ({len(eval_data)} samples).",
        "\n## Evaluation Metrics Summary\n",
        "| Model | Retrieval Precision@3 | ROUGE-L F1 Score | BERTScore F1 | Avg Inference Time (s) |",
        "|---|---|---|---|---|"
    ]
    
    model_keys = ["mistral_base", "mistral_tuned", "qwen_tuned"]
    model_labels = {
        "mistral_base": "Mistral-7B-Instruct (Base + RAG)",
        "mistral_tuned": "Mistral-7B-Instruct (Tuned + RAG)",
        "qwen_tuned": "Qwen-2.5-7B-Instruct (Tuned + RAG)"
    }
    ...
    for name in model_keys:
        if name in results:
            res = results[name]
            report_lines.append(
                f"| **{model_labels[name]}** | {res['precision']:.4f} | {res['rouge_l']:.4f} | {res['bert_score']:.4f} | {res['avg_time']:.2f}s |"
            )
            
    report_lines.append("\n## Analysis and Verdict\n")
    if "mistral_base" in results and "mistral_tuned" in results and "qwen_tuned" in results:
        mb_score = results["mistral_base"]["bert_score"]
        mt_score = results["mistral_tuned"]["bert_score"]
        qt_score = results["qwen_tuned"]["bert_score"]
        
        best_score = max(mb_score, mt_score, qt_score)
        if best_score == mb_score:
            winner = "Mistral-7B-Instruct (Base + RAG)"
        elif best_score == mt_score:
            winner = "Mistral-7B-Instruct (Tuned + RAG)"
        else:
            winner = "Qwen-2.5-7B-Instruct (Tuned + RAG)"
            
        report_lines.append(
            f"Based on local semantic evaluation (BERTScore) over all validation samples, the winner is **{winner}**.\n\n"
            f"- **Mistral-7B-Instruct (Base + RAG)** BERTScore F1: `{mb_score:.4f}`\n"
            f"- **Mistral-7B-Instruct (Tuned + RAG)** BERTScore F1: `{mt_score:.4f}`\n"
            f"- **Qwen-2.5-7B-Instruct (Tuned + RAG)** BERTScore F1: `{qt_score:.4f}`\n"
        )
    else:
    ...
    sample_indices = [0, len(eval_data)//2, len(eval_data)-1] # Print comparison for three distributed questions
    for idx in sample_indices:
        if idx < len(eval_data):
            ...
            if "mistral_base" in results:
                report_lines.append(f"**Mistral-7B Base Answer**:\n> {results['mistral_base']['answers'][idx]}\n")
            if "mistral_tuned" in results:
                report_lines.append(f"**Mistral-7B Tuned Answer**:\n> {results['mistral_tuned']['answers'][idx]}\n")
            if "qwen_tuned" in results:
                report_lines.append(f"**Qwen-2.5-7B Tuned Answer**:\n> {results['qwen_tuned']['answers'][idx]}\n")
            report_lines.append("---\n")
            
    # Save the report
    report_path = "data/validation_split_evaluation_report.md"
